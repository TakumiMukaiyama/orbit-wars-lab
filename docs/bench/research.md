...
未確定点もあります。Orbit Wars はまだ新しく、現時点では Halite や Lux のような「最終上位解法の公開 write-up が揃っている状態」ではありません。公開 notebook は確認できましたが、Kaggle 側のページ制限で本文詳細まではこの調査で十分に取得できませんでした。そのため、**現時点の具体的メタ**、たとえば「軌道惑星 rush がどこまで通るか」「2p と 4p の leaderboard 比重」「彗星偏重が直近で強いか」については、今後の公開対戦・上位 notebook・discussion を追って更新する余地があります。とはいえ、上で述べた設計原則自体は、公式環境仕様と過去の近縁研究の両方から見て、かなり堅いです。citeturn49search0turn38search0turn38search1turn43search0turn44search0