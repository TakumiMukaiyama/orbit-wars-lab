...
## P5確定 + P6: forward-sim board_value (採用)

- 変更: CENTRAL_BONUS_MAX=40.0 に確定 (A/B テストより元値採用)、`estimate_hold_turns()` を `world.py` に追加し、`enumerate_candidates()` の静止惑星 value 計算を `rival_eta` ベースから `hold_turns` ベースに変更
- Run: `runs/2026-04-28-001`
- 単体テスト: 165 passed, 2 skipped
- Commits: `42d9e50` (world), `4345304` (targeting)

| 相手 | run024 (P5基準, 58.9%) | P6今回 (001) | Δ |
|---|---|---|---|
| baselines/nearest-sniper | 10-0-0 (100%) | 10-0-0 (100%) | = |
| baselines/random | 10-0-0 (100%) | 10-0-0 (100%) | = |
| baselines/starter | 10-0-0 (100%) | 10-0-0 (100%) | = |
| external/kashiwaba-rl | 10-0-0 (100%) | 10-0-0 (100%) | = |
| external/pilkwang-structured | 2-8-0 (20%) | 5-5-0 (50%) | **+30pp** |
| external/sigmaborov-reinforce | 3-7-0 (30%) | 3-7-0 (30%) | = |
| external/sigmaborov-starter | 4-6-0 (40%) | 7-3-0 (70%) | **+30pp** |
| external/tamrazov-starwars | 2-8-0 (20%) | 2-8-0 (20%) | = |
| external/yuriygreben-architect | 2-8-0 (20%) | 3-7-0 (30%) | +10pp |
| **TOTAL** | **53/90 (58.9%)** | **60/90 (66.7%)** | **+7.8pp** |

判定: +7.8pp、採用基準 (+2pp) を大きく上回る。**採用**。

## 遊兵分析: idle ships の発生原因分類 (2026-04-29)

P7 後の `runs/2026-04-28-007` (30 敗) に対して、「発射しなかったターンの自艦」が
なぜ遊兵になっているかを分類した。

### 分析方法

replay JSON から「自艦 > reserve かつ picked=None (何も発射しなかったターン)」を抽出し、
以下 2 種類に分類するスクリプトを作成:

- `cant_afford`: val>0 の候補は存在するが single planet での avail が ships_needed 未満
- `all_val_leq0`: 候補が全て value<=0 で select_move が None を返した

### 結果 (30 敗合計)

| 分類 | idle 艦数 | 割合 |
|---|---|---|
| `cant_afford` (val>0 だが single planet で払えない) | 305,343 | 98.9% |
| `all_val_leq0` (全候補 value<=0) | 3,384 | 1.1% |
| 合計 | 308,727 | 100% |

**98.9% が `cant_afford`**: 遊兵の大部分は「撃ちたいターゲットはあるが 1 惑星では ship が足りない」
状態。value 計算の問題ではない。`all_val_leq0` の 1.1% はほぼ `hold=0` (相手が先着するケース) で
これは正しい動作。

結論: single-planet 発射 gate (`avail >= ships_needed`) が主なボトルネック。
解決策として partial send を試みた (下記参照)。

---

## Partial Send 実験 (不採用・revert 済み, 2026-04-29)

### 試行内容

`select_move` の発射ゲートを「avail >= ships_needed」から「avail >= 1 なら partial send」に変更:
...
if my_planet.ships - reserve < ships_needed:
    continue
# AFTER (試行・revert 済み)
avail = my_planet.ships - reserve
if avail < 1:
    break
send = min(avail, ships_needed)
```

単体テスト: `test_reserve_blocks_small_stockpile` を partial send 期待値に合わせて更新。

### 結果

- 2P gauntlet: 約 **43% WR** (P7 後 74.4% から大幅後退)
- 単体テストは pass

### 失敗の原因: sequential combat モデルとの非互換

orbit-wars の戦闘ルールでは、同一惑星に到着した複数フリートは「先着順に garrison と
1 対 1 で戦う」。同ターン着弾でなければ合算されない。

match 051 の観察:
- t=2-23 の間、mine が毎ターン 3 ships を同じ軌道惑星 (garrison 71) に送り続ける
- 発射総艦数 63 ships だが 3 ships ずつ順次吸収され、一度も占領できず
- mine は t=39 まで 1 惑星のまま (P7 後は ships を蓄積してから一括送付し、展開できていた)

### 制約として確立した事実

| 制約 | 内容 |
|---|---|
| partial send は非機能 | 小艦隊を先行発射しても先着順で吸収される |
| swarm の ETA 条件は厳格 | 着弾ターン差 1-2 以内でなければ先着 fleet が消耗する |
| ships_needed を満たして 1 撃が基本 | single-planet で全額払えるようになるまで溜める現行方針は正しい |

この制約は P6 swarm 試行での失敗とも一致する (ships 不足惑星を swarm で消費して共倒れ)。
...
## 2026-05-04: Phase 1d (Vadasz 観察由来 7 機能)

ブランチ: `feature/phase1d-vadasz-features`

実装機能:
- 容量ダンプ (capacity dump): 上限手前で強制射出し生産停止を防ぐ
- 後方押し出し (rear push): 後方安全惑星から前線惑星へ艦を流す
- JIT 逆算送出 (JIT dispatch): 到着ぎりぎりまで援軍送出を遅らせる
- 防衛放棄判断 (abandon defense): 守備コスト > 生産価値 * 1.5 なら doomed 扱い
- 出撃直後スナイプ (post-launch snipe): 敵出撃後の手薄惑星を全モードでスナイプ
- 同時並行占領 (concurrent expansion): ETA が CONCURRENT_WINDOW 以内の候補にボーナス
- N>2 swarm + overcap: 3 惑星同時着弾対応、最小必要量に OVERCAP_FACTOR=1.2 を乗算

テスト: 218 passed, 2 skipped (軌道計算の既知スキップ)
動作確認: `env.run([agent, 'random'])` で 500 ステップ完走を確認
