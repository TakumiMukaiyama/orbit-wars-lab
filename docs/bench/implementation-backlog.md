...
**2026-04-29 実測結果** (`runs/2026-04-28-007`, 30 敗合計):
...
- **value<=0 は主因ではない** (1.1%)。`all_val_leq0` のほぼ全ては `hold=0` (相手が先着確定) で、これは正しい動作。
- **主因は `cant_afford`**: single planet の `avail < ships_needed` ゲートに引っかかり続けている。

**Partial send 試行結果 (不採用)**:

`select_move` を `send = min(avail, ships_needed)` に変更したが 43% WR (74.4% から -31pp)。
根本原因: orbit-wars は sequential combat モデルのため、ships_needed 未満の小艦隊は
garrison に吸収される。3 ships × 22 ターン = 66 ships 全滅 (garrison 71 に到達できず)。

**設計制約として確立**: `avail >= ships_needed` ゲートは変えない。遊兵対策は
「払えるようになるまで蓄積を早める」方向 = logistics / reinforcement が正解。
