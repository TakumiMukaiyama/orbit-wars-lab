                ...
                if (
                    ...
                    for turn in range(1, fall_turn + 1):
                        if owner_at.get(turn) != self.player:
                            deficit_hint = max(
                                deficit_hint, int(math.ceil(ships_at.get(turn, 0))) + 1
                            )
                            break
...
def build_reinforcement_missions(world, planned_commitments, modes, source_budget_fn):
    ...
    for target_id, info in world.threatened_candidates.items():
        ...
        for src in world.my_planets:
            ...
            probe_ships = max(
                PARTIAL_SOURCE_MIN_SHIPS, int(info["deficit_hint"]) + REINFORCE_SAFETY_MARGIN
            )
...
def plan_moves(world):
        ...
        if mission.kind in ("single", "snipe", "reinforce", "crash_exploit"):
            ...
            if mission.kind in ("snipe", "crash_exploit"):
                ...
                send = min(
                    ...
                    max(
                        missing,
                        preferred_send(target, missing, arrival_turn, send_limit, world, modes),
                    ),
                )
            if send < missing:
                continue
    ...
    if (
        ...
        safe_fronts = [
            planet for planet in world.my_planets if planet.id not in world.doomed_candidates
        ]
        if safe_fronts:
